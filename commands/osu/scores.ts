...
module.exports = async (interaction: ChatInputCommandInteraction) => {
    ...
    let user = isNaN(query) ? user_osu.id : query;
    if (!user) return interaction.editReply(`User **${user}** not found!`);

    let scores_ = await osu.getBeatmapScores(beatmap_id, user, mode);
    if (scores_.error) return interaction.editReply(scores_.error);
    if (scores_.length == 0) return interaction.editReply(`No scores found for **${user}** on **${beatmap_id}**.`);
    ...
    let lines = [
        { separator: ' ', indent: '', content: [`**Scores for [${user_osu.username}](https://osu.ppy.sh/users/${user_osu.id})**`] },
        { separator: '', indent: '', content: ['\u200b'] },
        personal_best ? {
            separator: ' ', indent: '',
            content: [`🏅 **#${personal_best} Personal Best**`]
        } : null,
        {
            separator: ' • ', indent: '> ',
            content: [
        ...
        ...scores.slice(1, scores.length).map(sc => ({
            separator: ' • ', indent: '> ',
            content: [
                `${tools.getEmote(sc.rank).emoji} **[+${sc.modstr}](https://osu.ppy.sh/scores/osu/${sc.best_id})** (${format(sc.pps.stars, '0,0.00')}★)`,
